...
import { STATUS_LIST } from '@core/constants/status';
import {
    BarChart,
    Bar,
    XAxis,
...
export default function SeedingList() {
    const { seedings, deleteSeeding, globalSearch, setGlobalSearch, brands, fetchData } = useContext(AppContext);
    ...
    const handleDelete = async (id) => {
        ...
        */
        ...
        // 테스트용: 전체 삭제 가능
        if (window.confirm('선택한 요청을 정말 삭제하시겠습니까? (테스트용: 모든 상태 삭제 가능)')) {
    ...
    const handleBulkDelete = async () => {
        ...
        */
        ...
        let confirmMsg = `선택한 ${selectedIds.length}건을 모두 삭제하시겠습니까? (테스트용: 모든 상태 삭제 가능)`;
            ...
            <div className="card" style={{ padding: '24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 0 4px', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9375rem', color: 'var(--text-muted)' }}>
                        ...
                        <span>전체 <strong style={{ color: 'var(--text-main)' }}>{totalCount}</strong>건</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        <button className="btn btn-outline" style={{ padding: '6px 14px' }} disabled={!hasNext || page >= totalPages} onClick={() => setPage(p => p + 1)}>›</button>
                        <button className="btn btn-outline" style={{ padding: '6px 14px' }} disabled={!hasNext || page >= totalPages} onClick={() => setPage(totalPages)}>»</button>
                    </div>
                </div>
            </div>
        </>

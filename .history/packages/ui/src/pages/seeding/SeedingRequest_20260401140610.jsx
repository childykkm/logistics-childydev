...
export default function SeedingRequest() {
    ...
    const handleFileUpload = async () => {
        ...
        if (noCodeRows.length > 0) {
            ...
            if (!window.confirm('시딩 요청을 등록하시겠습니까?')) return;
        }
        ...
        // excelPreview 데이터를 JSON 배열로 변환 (validRows 이미 위에서 선언됨)
        const payload = validRows.map(row => ({
            ...
            status: (row.expectedStock !== '-' && row.expectedStock < 0) ? 'reviewing' : 'pending',
        }));

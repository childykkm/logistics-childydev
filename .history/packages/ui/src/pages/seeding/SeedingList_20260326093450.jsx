...
import { Filter, Download, Trash2, Edit, TrendingUp, BarChart2, PieChart as PieChartIcon, AlertCircle } from 'lucide-react';
...
export default function SeedingList() {
    const { seedings, deleteSeeding, globalSearch, brands, statuses } = useContext(AppContext);
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case '대기': return <span className="badge badge-warning">대기</span>;
            case '검토중': return <span className="badge badge-warning" style={{ background: '#FFFBEB', color: '#B45309' }}>검토중</span>;
            case '승인': return <span className="badge badge-info">승인</span>;
            case '발주완료': return <span className="badge badge-success" style={{ background: '#CCFBF1', color: '#0F766E' }}>발주완료</span>;
            case '출고완료': return <span className="badge" style={{ background: '#EDE9FE', color: '#6D28D9' }}>출고완료</span>;
            case '반려': return <span className="badge badge-danger">반려</span>;
    ...
    const handleBulkDelete = () => {
        ...
        if (window.confirm(`선택한 ${selectedIds.length}건의 요청을 모두 삭제하시겠습니까?`)) {
            selectedIds.forEach(id => deleteSeeding(id));
            setSelectedIds([]);
            alert('삭제되었습니다.');
        }
    };
                    ...
                    <table className="table" style={{ minWidth: '2500px' }}>
                        <thead>
                            <tr style={{ whiteSpace: 'nowrap' }}>
                                ...
                                <th>수취인명</th>
                                <th>수취인 우편번호</th>
                                <th style={{ textAlign: 'left' }}>수취인 주소</th>
                                <th>수취인 휴대폰</th>
                                <th>주문번호(쇼핑몰)</th>
                                <th>판매가(쇼핑몰)</th>
                                <th>결제금액(쇼핑몰)</th>
                        ...
                        <tbody>
                            {filteredData.map((row) => (
                                <tr key={row.id}>
                                    ...
                                    <td style={{ minWidth: '80px', whiteSpace: 'nowrap' }}>{getStatusBadge(row.status)}</td>
                                    <td
                                    ...
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                            {row.id}
                                            {row.hasStockIssue && <AlertCircle size={14} color="var(--danger)" title="재고 부족 품목 포함" />}
                                        </div>
                                    </td>

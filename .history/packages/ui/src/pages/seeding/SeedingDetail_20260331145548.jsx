...
export default function SeedingDetail() {
    ...
    const handleUpdate = async () => {
        try {
            console.log('[SeedingDetail] 저장 시도 - id:', data.id, 'payload:', JSON.stringify(data));
            await updateSeeding(data.id, data);
            console.log('[SeedingDetail] 저장 성공');
            alert('시딩 요청이 수정되었습니다.');
            navigate('/seeding');
        } catch(e) {
            console.error('[SeedingDetail] 저장 실패:', e);
